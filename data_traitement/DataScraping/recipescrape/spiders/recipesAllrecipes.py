...
FIELDS = [
    ...
    'name', 'img_data_url','category', 'summary', 'rating', 'rating_count', 'review_count', 'ingredients', 'directions', 'prep', 'cook', 'total', 'servings', 'yield',
...
class RecipeSpider(CrawlSpider):
    ...
    # add the start of all url that we want to visit
    start_urls = [
   ...
   # add the custom settings form extracting csv and jsonl files
    custom_settings = {
    ...
    def parse_item(self, response):
        ...
        try:
            img_data_url = response.xpath('.//div[@class="image-container"]/div/@data-src').extract_first()
            if(img_data_url == None ):
                img_data_url =  img_data_url = response.xpath('.//aside[@class="recipe-tout-image recipe-info-items-3"]/div/@data-src').extract_first()
                if(img_data_url == None ):
                    img_data_url =  img_data_url = response.xpath('.//aside[@class="recipe-tout-image recipe-info-items-2"]/div/@data-src').extract_first()
        except:
            img_data_url = "no image"
        # extract category from breadcrumb url
        breadcrumb_links = response.css(
            '.breadcrumbs__title::text').getall()
        try:
            category = []
            category.append(breadcrumb_links[2])
            category.append(breadcrumb_links[3])
        ...
        dataDic = {}
        dataDic.update(data)
        dataDic.update(recipe_meta)
        dataDic.update(nutrients_list)
